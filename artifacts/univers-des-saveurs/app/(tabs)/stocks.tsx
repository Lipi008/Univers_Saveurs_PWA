    ...
    <View style={[styles.productCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
      ...
      {editable && (
        <View style={styles.productActions}>
          ...
          <Pressable testID={`edit-stock-${product.id}`} onPress={onStock} style={[styles.stockPillBtn, { backgroundColor: colors.primary }]}>
            <Feather name="layers" size={14} color={colors.primaryForeground} />
            <Text style={[styles.stockBtnLabel, { color: colors.primaryForeground }]}>Stock</Text>
          </Pressable>
        </View>
      )}
    </View>
...
export default function StocksScreen() {
  ...
  const visibleProducts = useMemo(() => {
    let list = filterCategoryId ? products.filter((p) => p.category.id === filterCategoryId) : products;
  ...
  const pickImageFrom = async (onPick: (asset: ImagePicker.ImagePickerAsset) => void, onError: (msg: string) => void) => {
    ...
    if (!contentType || !ALLOWED_TYPES.includes(contentType as AllowedType)) { onError('Sélectionnez une image JPEG, PNG ou WebP.'); return; }
    if (asset.fileSize !== undefined && asset.fileSize > MAX_IMAGE_BYTES) { onError("L'image doit faire au maximum 5 Mo."); return; }
    onError('');
    onPick({ ...asset, mimeType: contentType });
  };

  const pickEditImage = () => pickImageFrom((a) => setEditImage(a), (e) => setEditError(e));
  ...
  const submitEditProduct = async () => {
    if (!editProduct || !isOnline) { setEditError('La modification nécessite une connexion.'); return; }
    const trimmed = editName.trim();
    if (!trimmed) { setEditError('Le nom est obligatoire.'); return; }
    if (!editCategoryId) { setEditError('Sélectionnez une catégorie.'); return; }
    const priceError = validateInteger(editPrice, 'Le prix');
    if (priceError) { setEditError(priceError); return; }
    setEditBusy(true);
    setEditError('');
    try {
      let imagePath: string | null = editProduct.imageUrl ?? null;
      if (editImage) {
        const contentType = (editImage.mimeType ?? 'image/jpeg') as AllowedType;
        const blob = await (await fetch(editImage.uri)).blob();
        if (blob.size > MAX_IMAGE_BYTES) throw new Error("L'image doit faire au maximum 5 Mo.");
        const upload = await requestUpload.mutateAsync({
          data: { name: editImage.fileName ?? `produit-${Date.now()}.jpg`, size: editImage.fileSize ?? blob.size, contentType },
        });
        const response = await fetch(upload.uploadURL, { method: 'PUT', headers: { 'Content-Type': contentType }, body: blob });
        if (!response.ok) throw new Error("Le téléversement de l'image a échoué.");
        imagePath = upload.objectPath;
      ...
      await updateProduct.mutateAsync({
        id: editProduct.id,
        data: { name: trimmed, categoryId: editCategoryId, price: Number(editPrice), imagePath },
      });
      await queryClient.invalidateQueries({ queryKey: getGetProductsQueryKey() });
      setEditProduct(null);
    } catch (error) {
      setEditError(`Modification impossible. ${errorMessage(error, 'Réessayez.')}`);
    } finally {
      setEditBusy(false);
    }
  };
  ...
  return (
    <View style={[styles.screen, { backgroundColor: colors.background, paddingTop: Platform.OS === 'web' ? 67 : insets.top }]}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {categories.length > 0 && (
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filterRow}>
            <Pressable onPress={() => setFilterCategoryId('')} style={[styles.filterChip, { backgroundColor: !filterCategoryId ? colors.primary : colors.card, borderColor: !filterCategoryId ? colors.primary : colors.border }]}>
              <Text style={[styles.filterChipText, { color: !filterCategoryId ? colors.primaryForeground : colors.foreground }]}>Tout</Text>
            </Pressable>
            {categories.map((cat) => (
              <Pressable key={cat.id} onPress={() => setFilterCategoryId(cat.id === filterCategoryId ? '' : cat.id)} style={[styles.filterChip, { backgroundColor: cat.id === filterCategoryId ? colors.primary : colors.card, borderColor: cat.id === filterCategoryId ? colors.primary : colors.border }]}>
                <Text style={[styles.filterChipText, { color: cat.id === filterCategoryId ? colors.primaryForeground : colors.foreground }]}>{cat.name}</Text>
              </Pressable>
            ))}
          </ScrollView>
        )}
        <View style={styles.productsHeader}>
          <Text style={[styles.sectionTitle, { color: colors.foreground }]}>
            Produits ({(search || filterCategoryId) ? `${visibleProducts.length}/${products.length}` : products.length})
          </Text>
          {canManage && isOnline && categories.length > 0 && (
            <Pressable onPress={() => setShowCategoryPanel((v) => !v)} style={[styles.manageCatBtn, { backgroundColor: colors.secondary }]}>
              <Feather name="grid" size={14} color={colors.primary} />
              <Text style={[styles.manageCatText, { color: colors.primary }]}>Catégories</Text>
              <Feather name={showCategoryPanel ? 'chevron-up' : 'chevron-down'} size={14} color={colors.mutedForeground} />
            </Pressable>
          )}
        </View>
        {canManage && isOnline && showCategoryPanel && categories.length > 0 && (
          <View style={[styles.categoryPanel, { backgroundColor: colors.card, borderColor: colors.border }]}>
            <View style={styles.categoryPanelHeader}>
              <Text style={[styles.categoryPanelTitle, { color: colors.foreground }]}>Gérer les catégories</Text>
              <Text style={[styles.categoryPanelHint, { color: colors.mutedForeground }]}>Renommer ou retirer sans supprimer les produits</Text>
            </View>
            {categories.map((category) => (
              <View key={category.id} style={[styles.categoryManageRow, { borderTopColor: colors.border }]}>
      ...
      <Modal visible={!!editProduct} animationType="slide" transparent onRequestClose={() => !editBusy && setEditProduct(null)}>
        <View style={styles.backdrop}><View style={[styles.sheet, { backgroundColor: colors.background }]}>
          <View style={styles.modalHeader}><Text style={[styles.modalTitle, { color: colors.foreground }]}>Modifier le produit</Text><Pressable disabled={editBusy} onPress={() => setEditProduct(null)}><Feather name="x" size={22} color={colors.foreground} /></Pressable></View>
          <ScrollView keyboardShouldPersistTaps="handled">
            <Text style={[styles.label, { color: colors.foreground }]}>Nom du produit</Text>
            <TextInput value={editName} onChangeText={setEditName} placeholder="Nom du produit" placeholderTextColor={colors.mutedForeground} style={[styles.input, { color: colors.foreground, borderColor: colors.border }]} />
            <Text style={[styles.label, { color: colors.foreground }]}>Catégorie</Text>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.optionRow}>
              {categories.map((cat) => <Pressable key={cat.id} onPress={() => setEditCategoryId(cat.id)} style={[styles.option, { backgroundColor: cat.id === editCategoryId ? colors.primary : colors.card, borderColor: colors.border }]}><Text style={{ color: cat.id === editCategoryId ? colors.primaryForeground : colors.foreground }}>{cat.name}</Text></Pressable>)}
            </ScrollView>
            <Text style={[styles.label, { color: colors.foreground }]}>Prix (F CFA)</Text>
            <TextInput value={editPrice} onChangeText={setEditPrice} keyboardType="numeric" placeholder="0" placeholderTextColor={colors.mutedForeground} style={[styles.input, { color: colors.foreground, borderColor: colors.border }]} />
            <Pressable onPress={() => { void pickEditImage(); }} style={[styles.imagePicker, { borderColor: colors.border, backgroundColor: colors.card }]}>
              {editImage
                ? <ExpoImage source={{ uri: editImage.uri }} style={styles.preview} contentFit="cover" />
                : editProduct?.imageUrl
                  ? <><ExpoImage source={{ uri: resolveImageUrl(editProduct.imageUrl) ?? '' }} style={styles.preview} contentFit="cover" /><View style={styles.changeImageOverlay}><Feather name="camera" size={20} color="#fff" /><Text style={styles.changeImageText}>Changer l'image</Text></View></>
                  : <><Feather name="image" size={22} color={colors.primary} /><Text style={[styles.imageText, { color: colors.foreground }]}>Ajouter une image (facultatif)</Text><Text style={[styles.hint, { color: colors.mutedForeground }]}>JPEG, PNG ou WebP · 5 Mo maximum</Text></>
              }
            </Pressable>
            {editError ? <Text style={styles.error}>{editError}</Text> : null}
            <Pressable disabled={editBusy} onPress={() => { void submitEditProduct(); }} style={[styles.submit, { backgroundColor: colors.primary, opacity: editBusy ? 0.6 : 1 }]}>
              {editBusy ? <View style={styles.busyRow}><ActivityIndicator color={colors.primaryForeground} /><Text style={[styles.submitText, { color: colors.primaryForeground }]}>Enregistrement…</Text></View> : <Text style={[styles.submitText, { color: colors.primaryForeground }]}>Enregistrer les modifications</Text>}
            </Pressable>
          </ScrollView>
        </View></View>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  permission: { flexDirection: 'row', alignItems: 'center', gap: 10, borderRadius: 14, padding: 13, marginBottom: 14 }, permissionText: { flex: 1, fontSize: 12, lineHeight: 18 }, alert: { flexDirection: 'row', alignItems: 'center', gap: 10, borderWidth: 1, borderRadius: 15, padding: 13, marginBottom: 22 }, alertText: { fontSize: 13, fontWeight: '700' }, sectionTitle: { fontSize: 17, fontWeight: '800' },
  ...
  filterRow: { gap: 8, paddingVertical: 4, marginBottom: 14 }, filterChip: { borderWidth: 1, borderRadius: 20, paddingHorizontal: 14, paddingVertical: 8 }, filterChipText: { fontSize: 12, fontWeight: '700' },
  productsHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }, manageCatBtn: { flexDirection: 'row', alignItems: 'center', gap: 5, borderRadius: 10, paddingHorizontal: 11, paddingVertical: 7 }, manageCatText: { fontSize: 12, fontWeight: '700' },
  productCard: { borderWidth: 1, borderRadius: 16, padding: 12, marginBottom: 10, flexDirection: 'row', alignItems: 'center', gap: 11 }, productThumb: { width: 58, height: 58, borderRadius: 13, alignItems: 'center', justifyContent: 'center' }, productDetails: { flex: 1, minWidth: 0 }, productName: { fontSize: 14, fontWeight: '800' }, productCategory: { fontSize: 11, marginTop: 3 }, productPrice: { fontSize: 13, fontWeight: '800', marginTop: 4 }, stockStatus: { fontSize: 11, fontWeight: '700', marginTop: 3 }, productActions: { flexDirection: 'row', alignItems: 'center', gap: 7, flexShrink: 0 }, actionIconBtn: { width: 36, height: 36, borderRadius: 10, alignItems: 'center', justifyContent: 'center' }, stockPillBtn: { height: 36, borderRadius: 10, paddingHorizontal: 11, flexDirection: 'row', alignItems: 'center', gap: 5 }, stockBtnLabel: { fontSize: 12, fontWeight: '700' }, changeImageOverlay: { position: 'absolute', bottom: 0, left: 0, right: 0, backgroundColor: 'rgba(0,0,0,0.45)', alignItems: 'center', justifyContent: 'center', paddingVertical: 8, flexDirection: 'row', gap: 6 }, changeImageText: { color: '#fff', fontSize: 12, fontWeight: '700' },
